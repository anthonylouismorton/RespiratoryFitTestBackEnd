...
async function getRespiratorModels(company){
    try {
        let pool = await sql.connect(config);
        let respiratorModels = await pool.request()
        .input('respiratorManufacturer', sql.NVarChar, company)
        .execute('SelectRespiratorModels')
        return respiratorModels.recordsets[0];
    }
    catch(err){
        return(err)
    };
...
module.exports = {
    ...
    getRespiratorModels: getRespiratorModels
};
